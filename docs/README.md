...
* 식당 예상 방문 날짜 입력 받기
    * InputView로부터 식당 예상 방문 날짜 입력(String 타입)을 전달 받는다.
    * 사용자 입력 검증을 진행한다.
        * 숫자를 입력받아야 한다.
    * 날짜 객체로 변환한다.
    * 유효하지 않은 입력인 경우 에러 메세지 출력 후 재입력을 받는다.
        * 에러메세지 양식
            ```
...
* 주문 메뉴와 개수 입력 받기
    * InputView로부터 주문 메뉴와 개수(String 타입)을 전달 받는다.
    * 주문 메뉴와 개수는 `,`(콤마) 단위로 구분한다.
    * 사용자 입력 검증을 진행한다.
        * 주문 메뉴와 개수의 입력 양식은 `{주문 메뉴}-{개수}`여야 한다.
        * 개수는 정수여야 한다.
    * 주문 메뉴 객체로 변환한다.
    * 유효하지 않은 입력인 경우 에러 메세지 출력 후 재입력을 받는다.
        * 에러메세지 양식
