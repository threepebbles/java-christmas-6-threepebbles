...
고정된 템플릿에 OutputController로부터 받은 양식을 대입하여 출력한다.
...
비즈니스 로직에 사용되는 객체를 출력될 수 있는 타입(String 타입)으로 변환해주는 역할

* 공통사항
    * [x] 모든 금액은 한국 locale 형식으로 출력한다. (세자리 단위 콤마)

* [x] 이벤트 플래너 헤더 출력 양식을 생성한다.
    * 양식 예시
        ```
        {M}월 {D}일
        ```

* [x] 주문 메뉴의 출력 양식을 생성한다.
    * 양식 예시
        ```
        {메뉴1 이름} {메뉴1 개수}개
        {메뉴2 이름} {메뉴2 개수}개
        ```
* [x] 할인 전 총주문 금액의 출력 양식을 생성한다.
    * 양식 예시
        ```
        {할인 전 총주문 금액}{화폐 단위}
        ```
* [x] 증정 메뉴의 출력 양식을 생성한다.
    * 양식 예시
        ```
        {증정메뉴 이름} {개수}개
        ```
* [x] 혜택 내역의 출력 양식을 생성한다.
    * 양식 예시
        ```
...
* [x] 총 혜택 금액의 출력 양식을 생성한다.
    * 양식 예시
        ```
        -{총 혜택 금액}{화폐 단위}
        ```

* [x] 할인 후 예상 결제 금액의 출력 양식을 생성한다.
    * 양식 예시
        ```
        {할인 후 예상 결제 금액}{화폐 단위}
        ```

* [x] 부여된 이벤트 배지의 출력 양식을 생성한다.
    * 양식 예시
        ```
        {이벤트 배지명}
        ```
...
* 생성자 검증
    * 일(day)는 1~31 사이의 숫자여야 한다.

### 메뉴(Menu)

주문 가능한 메뉴의 정보를 담고 있는 enum 클래스
...
* 주문한 메뉴(음식)와 개수를 갖고 있다.
* 주문 목록은 이름순으로 정렬되어 있다. (테스트 편의 위해 정렬함)
* 검증
    * 메뉴판에 있는 메뉴여야 한다.
    * 주문 개수는 양수여야 한다.
    * 총 메뉴 개수의 합이 20이하여야 한다.
    * 음료만 주문하면 안 된다.
    * 중복된 메뉴가 있으면 안 된다.
...
할인 종류 별로 얼마나 할인 됐는지 계산하여 혜택 내역을 반환한다.

### 할인 종류

할인 종류 정보를 담고 있는 enum 클래스

* 할인 종류 이름에 대한 정보를 갖고 있다.
