...
Presenter를 이용하여 사용자에게 출력될 화면을 **업데이트**하거나 **출력**하는 역할
...
Presenter는 중괄호 `{}`의 양식을 생성한다.

* 날짜를 받아 날짜 양식을 생성한다.
    * 예시
        ```
        {M월 D일}에 우테코 식당에서 받을 이벤트 혜택 미리 보기!
        ```
* 주문 목록을 받아 주문 목록 양식을 생성한다.
    * 예시
        ```
        <주문 목록>
        {티본스테이크 1개
