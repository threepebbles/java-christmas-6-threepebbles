...
* 주문 목록 검증
    * [x] 주문 메뉴와 개수의 입력 포맷은 `{주문 메뉴}-{개수}`여야 한다.
    * [x] 개수는 정수여야 한다.

#### InputConverter

사용자 입력(String 타입)을 파싱해서 비즈니스 로직에서 사용하는 객체로 변환하는 역할
...
* [x] 식당 예상 방문 날짜 입력 받기
    ...
    * [x] InputConverter를 이용하여 날짜 객체로 변환한다.
...
* [x] 주문 목록 입력 받기
    ...
    * [x] InputValidator를 이용하여 검증한다.
    * [x] InputConverter를 이용하여 주문 목록 객체로 변환한다.
    * [x] 유효하지 않은 입력인 경우 에러 메세지 출력 후 재입력을 받는다.
        * 에러메세지 양식
            ```
...
* 할인 전 총주문 금액이 10,000원 미만인 경우, 이벤트를 하나도 적용하지 않는다.
  <br><br>

* 총주문 금액이 10,000원 이상부터 아래의 이벤트를 진행한다.
...
* [x] 할인 종류와 할인 금액을 갖고 있다.

### 혜택 내역(DiscountDetails)

* [x] 각 할인 이벤트 별로 얼마나 할인되었는지 정보를 갖고 있다.

### 이벤트 기획자(Planner)

이벤트에 필요한 정보들을 계산해내는 역할
