...
public class OrdersInputValidatorTest {
    ...
    void 주문_목록_입력_검증_테스트(String orderInput) {
        // when & then
        assertThatCode(() -> OrderInputValidator.getInstance().validate(orderInput)).doesNotThrowAnyException();
    }
    ...
    void 주문_목록_입력_검증_예외_테스트_올바르지_않은_주문_포맷(String orderInput) {
        // when & then
        assertThatThrownBy(() -> OrderInputValidator.getInstance().validate(orderInput))
